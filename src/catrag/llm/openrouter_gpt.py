...
def cache_response(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        # for case if request multiple answer. Only add the n too key_data when n is not 1, to prevent all hash_id is mismatch
        if n != 1:
            key_data["n"] = n
...
class CacheOpenRouterAI(BaseLLM):
    """OpenAI LLM implementation."""
    @classmethod
    def from_experiment_config(cls, global_config: BaseConfig) -> "CacheOpenRouterAI":
    ...
    def __init__(self, cache_dir, global_config, cache_filename: str = None,
        ...
        self.openai_client = OpenAI(base_url=self.llm_base_url, api_key=getenv("OPENROUTER_API_KEY"), http_client=client, max_retries=self.max_retries)
    ...
    def batch_infer(self, messages_list, max_completion_tokens=9192, max_workers=8, **kwargs):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for idx, messages in enumerate(messages_list):
                # Schedule the execution of the original self.infer method
                future = executor.submit(
                    self.infer,  # The original function
            ...
            for future in concurrent.futures.as_completed(future_to_index):
                ...
                try:
                    # Getting the result may raise an exception
                    response_message, metadata, cache = future.result()
                    # If your original infer returns a cache_hit, unpack three values and adjust accordingly.
                    results[idx] = (response_message, metadata, cache)  # Assume cache_hit=False if not provided
