...
class BaseConfig:
    ...
    qa_llm_model: str = field(
        default=None,
        metadata={"help": "Class name indicating which LLM model to use for QA task. If either qa_llm_model or qa_llm_base_url is none, mean using same LLM as retreival"}
    )
    qa_llm_base_url: str = field(
        default=None,
        metadata={"help": "Base URL for the LLM model for QA task. If either qa_llm_model or qa_llm_base_url is none, mean using same LLM as retreival"}
    )
    ...
    dataset: Optional[Literal['hotpotqa', 'hotpotqa_train', 'musique', '2wikimultihopqa', 'musique_simple', "hover_1", "hover_2"]] = field(
        default=None,
    ...
    cache_dir: str = field(
        default='short_sum_cache',
        metadata={"help": "Directory path that store the summary"}
    )
    
    overhead_bypass_cache: bool = field(
        default=False,
        metadata={"help": "Bypass cache for overhead evaluation"}
    )
    
    weak_weight: str = field(
        default=0.2,
        metadata={"help": "Weight to assign for weak seed node derive from NER. Refer to the (ϵ) of Symbolic Anchor in paper."}
    )
    
    llm_score_max_edge: int = field(
        default=15,
        metadata={"help": "The Max number of Edges for scoring after pruning"}
    )
    
    llm_score_batch: int = field(
        default=15,
        metadata={"help": "The number of neighbour would be scored in one LLM calling. Same as llm_score_max_edge, which is score all pruned neighbour of one seed phrase in one LLM calling"}
    )
